...
- **Intégration SafeFetch** : Pour éviter les pannes de requête lors du test ou de l'exécution hors Tauri (ex: dans un simple navigateur web) ou en cas de problème de chargement de l'IPC Tauri, toutes les requêtes d'API s'effectuent via le helper `safeFetch` défini dans [config.ts](file:///c:/Users/Mael/Documents/GitHub/Wardis-Client/src/store/config.ts).
- **Ports Vidéo** :
